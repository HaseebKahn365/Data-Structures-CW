...
void createPoints() {
    for (int i = 0; i < noOfXPoints; i++) {
        for (int j = 0; j < noOfYPoints; j++) {
            allPoints.push_back(Point(j, i)); //we are drawing horizontal lines first that is why...
        }
    }
}
...
void checkCollision() {
    for (int i = 0; i < carList.size(); i++) {
        for (int j = i + 1; j < carList.size(); j++) {// it will run 1 less than the size ie. up to <8
